  ...
  const items = [
    {
      ...
      sub: 'checkout-api, us-east-1',
      badge: 'Investigating',
      color: 'amber',
    },
    {
      title: 'Database Connections',
      sub: 'prod-db, eu-central-1',
      badge: 'Critical',
      color: 'rose',
    },
  ] as const;
    ...
    <div className="p-4">
      ...
      <section className="mb-6">
        <div className="flex items-center justify-between mb-2">
          <h3 className="text-xs font-medium text-[#9AA3AE] uppercase tracking-wider">
            Active Incidents
          </h3>
          <span className="text-xs bg-[#1e2530] text-white rounded-full px-2 py-0.5">
            {items.length}
          </span>
        </div>
        ...
        <div className="space-y-3">
          {items.map((it) => (
            <div
              key={it.title}
              className={`p-3 bg-[#1e2530] rounded-md border ${BORDER[it.color]}`}
            >
              <div className="flex items-start justify-between">
                ...
                <span
                  className={`text-xs px-1.5 py-0.5 rounded ${BADGE_BG[it.color]} ${BADGE_TEXT[it.color]}`}
                >
                  {it.badge}
                </span>
              </div>
      ...
      {/* Suggested Queries */}
      <section className="mb-6">
        <h3 className="text-xs font-medium text-[#9AA3AE] uppercase tracking-wider mb-2">
          Suggested Queries
        </h3>
        ...
        <div className="space-y-2">
          {['API Performance', 'Payment Processing'].map((d) => (
            <a
              key={d}
              href="#"
              className="block p-2 text-sm bg-[#1e2530] hover:bg-[#273041] rounded-md border border-[var(--border)] group"
            >
